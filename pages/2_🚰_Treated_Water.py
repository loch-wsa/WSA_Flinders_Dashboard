...
def display_category_section(category, data_df, treated_data, ranges_df, treated_ranges, week_num):
    ...
    if not category_params.empty:
        ...
        with st.expander(f"View {category} Parameters", expanded=False):
            ...
            for _, row in category_params.iterrows():
                ...
                # Use treated_data instead of data_df
                if pd.notna(row['ALS Lookup']) and row['ALS Lookup'] != '':
                    # Parameter has ALS Lookup
                    param_data = treated_data[treated_data['ALS Lookup'] == row['ALS Lookup']]
...
try:
    ...
    # Determine available weeks from the treated data
    ...
    for i, category in enumerate(main_categories):
        with cols[i % 2]:
            ...
            display_category_section(
                category,
                treated_data,  # Changed from influent_data to treated_data
                treated_data,
                treated_ranges,  # Changed from influent_ranges to treated_ranges
                treated_ranges,
                week_num
            )
